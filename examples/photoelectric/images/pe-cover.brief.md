...
- style: photo
- brief: 실험대 위에 놓인 유리 진공관 장치 위로 보랏빛 자외선이 비치고 은은한 빛이 반사되는 정갈한 실험실 정물 사진
...
3. 완성 프롬프트를 lesson.json에서 이 요청의 `image.prompt` 필드에 넣거나, 이 폴더에 `pe-cover.prompt.txt`로 저장한다(끝은 `AR 4:3`).

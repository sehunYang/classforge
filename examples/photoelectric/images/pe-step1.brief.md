...
- style: photo
- brief: 학생 손이 스위치를 눌러 자외선 램프를 켜는 순간, 유리 진공관 장치 위로 보랏빛 자외선이 비치기 시작하는 클로즈업
- state: 자외선 램프가 막 켜지며 유리 진공관 장치를 비추기 시작하는 상태
- mustShow: 막 켜지는 자외선 램프, 유리 진공관 장치
...
3. 완성 프롬프트를 lesson.json에서 이 요청의 `image.prompt` 필드에 넣거나, 이 폴더에 `pe-step1.prompt.txt`로 저장한다(끝은 `AR 4:3`).
